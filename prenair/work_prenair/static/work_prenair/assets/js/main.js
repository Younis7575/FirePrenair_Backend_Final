...
(function (win, $) {
  ...
  if ($("#map").length > 0) {
    ...
    markers.forEach(function (item) {
      var marker = L.marker([item.lat, item.lng], { icon: locateIcon }).addTo(
        map
      );
  ...
  const handleActiveMenu = function () {
    $(".tab_btn").each(function () {
      if ($(this).hasClass("active")) {
        ...
        if (indicator.length > 0) {
          indicator.css(
            "width",
            $(this)[0].getBoundingClientRect().width + "px"
          );
          indicator.css(
            "left",
            $(this)[0].getBoundingClientRect().left -
              $(this)[0].closest(".menu").getBoundingClientRect().left +
              "px"
          );
        }
      }
    ...
    $(".tab_btn").on("click", function () {
      ...
      const $section = $(this).closest(
        "section, .list_category, .list_tags, .list_pagination"
      );
  ...
  const handleFaq = function () {
    $(".faq_item .heading, .toggle_item .heading").on("click", function () {
      $(this)
        .closest(".faq_item, .toggle_item")
        .toggleClass("active")
        .siblings(".faq_item, .toggle_item")
        .removeClass("active");
      $(this)
        .closest(".faq_item, .toggle_item")
        .find(".answer, .toggle_menu")
        .slideToggle(300);
      $(this)
        .closest(".faq_item, .toggle_item")
        .siblings(".faq_item, .toggle_item")
  ...
  const handleAddFilter = function () {
    ...
    optionItems.on("click", function () {
      ...
      if (filterSection.length > 0) {
        ...
        $(
          '.list_filtered .list .selected_item[data-type="' + filterType + '"]'
        ).remove();
    ...
    checkboxItems.change(function () {
      ...
      if ($(this).is(":checked")) {
        ...
        $(".list_filtered .list")
          .find(`.selected_item[data-id="${selectedId}"]`)
          .remove();
      }
  ...
  const handlePriceRange = function () {
    ...
    minPrice.on("change", function () {
      ...
      if ($(this).val() < 0) {
        ...
        progress.css({
          left:
            ((maxValue - priceGap) / rangeInput.eq(0).attr("max")) * 100 + "%",
        });
      }
    });
    ...
    maxPrice.on("change", function () {
      ...
      if (parseInt($(this).val()) > rangeInput.eq(1).attr("max")) {
        ...
        parseInt($(this).val()) > minValue + priceGap &&
        parseInt($(this).val()) <= rangeInput.eq(1).attr("max")
      ) {
        rangeInput.eq(1).val($(this).val());
        progress.css({
          right:
            100 - ($(this).val() / rangeInput.eq(1).attr("max")) * 100 + "%",
        });
        ...
        progress.css({
          right:
            100 -
            ((minValue + priceGap) / rangeInput.eq(1).attr("max")) * 100 +
            "%",
        });
      }
    });
  ...
  const handleDisplayPreviewImage = function () {
    $("#uploadImage, #uploadLogo, #uploadBanner").on(
      "change",
      function (event) {
        ...
        $categoryList.on("click", "li", function () {
          ...
          if (!$selectedCategories.text().includes(selectedItem)) {
            $selectedCategories.append(
              '<span class="selected-category">' + selectedItem + "</span>"
            );
          }
          $input.val("");
          $categoryList.hide();
